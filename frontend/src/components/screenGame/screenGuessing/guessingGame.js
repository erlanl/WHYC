  ...
  useEffect(() => {
    const receiveImage = async () => {
      const check = await axios.post("http://localhost:5001/get_image", {
        ...
        await axios.post("http://localhost:5001/change_status", {
          "id": id,
          "room": codigo
        }).then(response2 => {
          setLoading(false);
      ...
      <div className='Tela'>
        ...
        <div className="fixed inset-0 flex flex-col items-center justify-center z-50">
          <div role="status" className="grid place-items-center">
            ...
            <p className="font-semibold text-white text-center custom-font-inter my-2">Calculando pontuação...</p>
          </div>
        </div>
      ) : (
        <div>
          <Resultado venceu={props.venceu} pontos={props.pontos} opPontos={props.opPontos} setLoading={setLoading}/>
          <NovoJogo />
        </div>
      )}
      </div>
...
function Resultado (props) {
  const func = async () => {
    const pontos = await axios.post("http://localhost:5001/redefine_game",  {
      "id": id,
      "room": codigo
    }).then(response => {
  ...
  const pts = func();
  if (pts.data.score > pts.data.opScore) {
      return (
          <section className='Resultado-Mensagem'>
              ...
              <Pontuacao pontos={pts.data.score} opPontos={pts.data.opScore} />
          </section>
      )
  } else if (pts.data.score < pts.data.opScore) {
      return (
          <section className="Resultado-Mensagem">
    ...
    return (
      <section className='Resultado-Mensagem'>
        <img src={iconLose} className='Resultado-Mensagem-Logo' alt='Tie icon' />
        <h1 className='Resultado-Mensagem-Texto'>Você empatou!</h1>
        <Pontuacao pontos={pts.data.score} opPontos={pts.data.opScore} />
      </section>
    )
  }
